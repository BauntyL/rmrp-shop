...
export const CarStep3: React.FC<CarStep3Props> = ({ data, onDataChange, onValidationChange }) => {
  ...
  const photoCategories = [
    { name: 'Общий вид', description: 'Фото автомобиля целиком' },
    { name: 'Салон', description: 'Передние и задние сиденья, приборная панель' },
    { name: 'Двигатель', description: 'Подкапотное пространство' },
    { name: 'Багажник', description: 'Багажное отделение' },
    { name: 'Диски и шины', description: 'Состояние колес' },
    { name: 'Документы', description: 'ПТС, СТС (закройте личные данные)' }
  ];

  return (
    <div className="space-y-6">
      <div className="text-center mb-6">
        <h3 className="text-lg font-semibold text-white mb-2">Фотографии автомобиля</h3>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-center w-full">
          <label htmlFor="car-images" className="flex flex-col items-center justify-center w-full h-32 border-2 border-white/20 border-dashed rounded-lg cursor-pointer bg-white/5 hover:bg-white/10 transition-colors">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {formData.images.map((image: any, index: number) => (
                <div key={image.id} className="relative group">
                  <div className={`relative rounded-lg overflow-hidden ${index === formData.mainImageIndex ? 'ring-2 ring-blue-400' : ''}`}>
                    <img
                      src={image.url}
      ...
      <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4">
        <h4 className="text-yellow-400 font-medium mb-2">💡 Советы для лучших фотографий:</h4>
        <ul className="text-yellow-300/80 text-sm space-y-1">
          ...
          <li>• Сфотографируйте все дефекты честно</li>
          <li>• Очистите автомобиль перед съемкой</li>
