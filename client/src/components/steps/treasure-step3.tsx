...
export default function TreasureStep3({ data, onDataChange, onValidationChange, servers }: TreasureStep3Props) {
  ...
  const updateData = (field: string, value: string | number) => {
    ...
    const isValid = hasContact && hasPrice && hasServer;
...
export default function TreasureStep3({ data, onDataChange, onValidationChange }: TreasureStep3Props) {
  const [formData, setFormData] = useState({
    images: data.images || [],
  ...
  return (
    <div className="space-y-6">
      <div className="text-center mb-6">
        <h3 className="text-lg font-semibold text-amber-400 mb-2">Фотографии и контакты</h3>
        <p className="text-slate-400">Добавьте качественные фото и контактную информацию</p>
      </div>
